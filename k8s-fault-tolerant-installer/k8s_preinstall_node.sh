...
# Add to /etc/hosts, only if not already present (as will be the case if this is executed with ansible)
...
then
	...
	echo "192.168.122.5  vm5" >> /etc/hosts
	echo "192.168.122.6  vm6" >> /etc/hosts
fi
...
# Replace https://packages.cloud.google.com/apt/doc/apt-key.gpg by https://dl.k8s.io/apt/doc/apt-key.gpg as per https://github.com/kubernetes/k8s.io/pull/4837#issuecomment-1446426585
